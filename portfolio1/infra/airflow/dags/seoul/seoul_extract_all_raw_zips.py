...
if [ "{{ params.mode }}" = "flat" ]; then
  if ls "${ROOT_DIR}/"*.csv 1>/dev/null 2>&1; then
    ...
    gsutil -m cp "${ROOT_DIR}/"*.xlsx "gs://{{ params.bucket }}/{{ params.extracted_prefix }}/"
  elif ls "${ROOT_DIR}/"*.xls 1>/dev/null 2>&1; then
    gsutil -m cp "${ROOT_DIR}/"*.xls "gs://{{ params.bucket }}/{{ params.extracted_prefix }}/"
  else
    echo "ERROR: no csv/xlsx/xls files found in ${ROOT_DIR}"
    exit 1
  fi
